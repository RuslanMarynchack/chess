...
function handlerClickFigureOnMove() {
    ...
    window.addEventListener("click", (e) => {
        ...
        if (isCellActive || cell.classList.contains(classes.attack)) {
            isDoublePawnMove = Math.abs((+cell.dataset.index - isSelectedIndex)) === 16;
...
function handlerPawn(pawn, cells, isAddClasses) {
    ...
    if (isDoublePawnMove && (!handlerFigureCurrentMove(pawn) || pawn.classList.contains(classes.enPassant))) {
        directionsLeftRight.forEach(direction => {
            const directionFigure = cells[direction + dataIndex].querySelector(selectors.figure);

            if (directionFigure) {
                const directionCellInner = directionFigure.closest(selectors.cellInner);
                ...
                if (
                    ...
                    directionCellInner.classList.contains(classes.lastMove) &&
...
function kingsAreLeft() {
    const figures = [...checkersWrapper.querySelectorAll(selectors.figure)];

    if (figures.length > 2) {
        return
    }

    const kings = figures.filter(figure => figure.classList.contains(classesFigure.king));
